...
class LocalDriver implements StorageDriver {
  ...
  constructor() {
    // Statically scoped to public/uploads. The env var may rename the leaf
    // folder but cannot point the driver outside the project.
    const leaf = path.basename(env().STORAGE_LOCAL_DIR)
    this.dir = path.join(process.cwd(), 'public', leaf)
    // Root-relative on purpose: next/image treats these as local assets (no
    // remotePatterns entry needed), and the app keeps working behind any host
    // or tunnel without re-deriving APP_URL.
    this.baseUrl = `/${leaf}`
  }

  /**
   * Keys are generated by contentKey (hex + extension), so anything containing
   * a separator or traversal segment did not come from us and must not be used
  ...
  async write(buffer: Buffer, mimeType: string): Promise<{ key: string; url: string }> {
    ...
    // Content-addressed: an existing file with this key is byte-identical.
    try {
